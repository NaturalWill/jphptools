...
class Json2xml {
	public static function xml_to_json($source) {
		...
		$json = json_encode ( array (
				$xml->getName () => $xml 
		) ); // php5，以及以上，如果是更早版本，请下载JSON.php
		return $json;
	}
	public static function json_to_xml($source, $charset = 'UTF-8') {
		if (empty ( $source )) {
			return false;
		}
		$array = json_decode ( $source ); // php5，以及以上，如果是更早版本，请下载JSON.php
		$xml = '<?xml version="1.0" encoding="' . $charset . '"?>';
		$xml .= self::_json_to_xml ( $array );
		return $xml;
	}
	static function _json_to_xml($source, $pkey = '') {
		$string = '';
